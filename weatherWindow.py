...
class WeatherWindow(PyQt5.QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.temperature_label = PyQt5.QtWidgets.QLabel(self)
        self.emoji_label = PyQt5.QtWidgets.QLabel(self)
        self.description_label = PyQt5.QtWidgets.QLabel(self)
        self.setUI()
    ...
    def setUI(self):
        ...
        #another font font-family: Noto Emoji;
    ...
    def handleWeather(self, data):
        ...
        # Time zone offset in seconds (UTC-5 hours)

        #test timezone
        # timezoneOffset = timezoneOffset / 3600
        # print(timezoneOffset)
        # timezone_hours = timezoneOffset# in seconds, 
        #now in hours

        # Calculate local time
        local_time = utc_now + timedelta(seconds=timezoneOffset)
        # print(local_time)
        # militTime = timedelta(seconds=timezoneOffset)
        # print(datetime(militTime).hour
        # print(militTime, "timezone")
        print(local_time.hour, "hours?")
        nighttime = False
        # print(f"Current time in the time zone with offset -18000 seconds: {local_time}")
        ...
        if currentWeatherImage == "Thunderstorm":
            self.emoji_label.setText("⛈️")
        if currentWeatherImage == "Drizzle":
            self.emoji_label.setText("⛆")
        if currentWeatherImage == "Rain":
            self.emoji_label.setText("🌧️")
        if currentWeatherImage == "Snow":
            self.emoji_label.setText("🌨️")
        if currentWeatherImage == "Atmosphere":
            self.emoji_label.setText("🌫️")
        if currentWeatherImage == "Clear":

            # self.emoji_label.setText("⛅")
            #determine clear for night/day

            if nighttime:
                self.emoji_label.setText("🌌")
            else:
                self.emoji_label.setText("☀️")
        if currentWeatherImage == "Clouds":
            self.emoji_label.setText("☁️")
            # ⛅
        # pass
